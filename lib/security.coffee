...
the mob. The conditions are:
 ...
 - `<`, `>`, '<=', '>=' - Less than, greater than. Equiv to `Number(<value>) > mob.get <attribute>`
...
class MaskFactory
    ...
    reMembership: ///
      ^
        ([+-]?)([\w.]+)         # <prefix:1><name:2>: Match just a group or name with no condition
        (?:
            ([<>]=?)(\d+        # <operator:3><value:4>: Match <, >, <=, >= with numbers
                (?:\.\d+)?      # Optional decimal point and numbers
            )
          | ([!~]?=)            # <operator:5><value:6,7,8>: Match ~=, !=, = with anything
            (?:
        ...
        (?:
            \s+(.*)             # <rest:10> Match any amount of whitespace before getting rest
          |                     # empty
         )
      $
    ///
    ...
    parse: (acl) ->
        ...
        while rest?.length
            ...
            if not mparts?
                throw new Error "ACL membership token '#{acl}' does not match membership expression"
