    ...
    describe 'Message', ->
        ...
        describe '#parse', ->
            ...
            it 'should parse format tokens', ->
                ...
                msg.parse('foo{bar} {baz} ').should.eql [
                    'foo', {token: '{', data: 'bar'}, ' ',
                    {token: '{', data: 'baz'}, ' ']
                msg.parse("{Name} hit{s} {^name}, killing {^him} while {he} defend{s} {himself}.").should.eql [
                    {token:'{', data:'Name'}, ' hit',
                    {token:'{', data:'s'}, ' ',
        ...
        describe '#forObserver', ->
            it 'should support specified observer', ->
                msg.message = "{Name} hit{s} {^name}, killing {^him} while {he} defend{s} {himself}."
                msg.forObserver(msg.observer).should.eql 'Foo hits bar, killing him while it defends itself.'
                msg.forObserver(msg.subject).should.eql 'You hit bar, killing him while you defend yourself.'
                msg.forObserver(msg.target).should.eql 'Foo hits you, killing you while it defends itself.'
